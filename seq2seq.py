...
def get_hidden_layer_output(lstm, X_train):
    # print("Num layers: "+str(len(lstm.model.layers)))
    # print(lstm.model.layers)
    # input()
...
def build_model(lstm):
    ...
    decoder_states_inputs = [Input(shape=(50,))]
...
if __name__ == "__main__":     
    ...
    X_train, y_train, X_val, y_val, X_test, y_test, embedding_matrix = lstm.create_Emb(1000)
    ...
    #gets classifier model output values
    print("Getting output from classifier")
    layer_output = get_hidden_layer_output(lstm, X_train)
    # y = copy.copy(X_train)
    
    print("Creating training and inference models")
